...
public class Helpers {
    ...
    // Waits for the OpenCart-style top alert / success or error banner and returns
    // its trimmed text.
    public static String getAlertTest(Page page) {
    ...
    public static double parseDouble(String priceText) {
        String cleanedPriceText = priceText.replaceAll("[^0-9.]", "");
        return cleanedPriceText.isEmpty() ? 0.0 : Double.parseDouble(cleanedPriceText);
    }

    // Retries a boolean-returning supplier until it returns true or the timeout
    // elapses.
    ...
    public static void autoAcceptDialog(Page page){
